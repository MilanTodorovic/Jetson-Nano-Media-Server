...
echo "This script is a wrapper around the jetson-image build process."
echo "It will call `just` and the corresponding scripts to build the rootfs and Jetson image."
echo "It additionally offers to compile an overclocked version of the kernel and copy it over to the flashed image."
...
git clone --depth=1 https://github.com/MilanTodorovic/jetson-image
...
cd ../jetson-image && just build-jetson-rootfs 20.04
...
if [[ "$build_overclock" == "y" ]]; then
    ...
    echo "Cloning jetson_nano_overclock"

    git clone -b $BRANCH --depth=1 https://github.com/MilanTodorovic/jetson_nano_overclock
    ...
    cd jetson_nano_overclock && ./build.sh
...
if [[ "$delete" == "y" ]]; then
    ...
    rm -rf /home/$USER/jetson_nano_overclock
    rm -rf /home/$USER/jetson-image
